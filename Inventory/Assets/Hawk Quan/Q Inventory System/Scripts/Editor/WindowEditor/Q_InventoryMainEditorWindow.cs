...
public class Q_InventoryMainEditorWindow : Q_InventoryEmptyEditorWindow
{
    ...
    private string[] titleBar = { "Main Editor", "Item Editor", "Crafting Editor", "Currency Editor", "Attribute Editor" };
    ...
    private void OnEnable()
    {
        if (EditorPrefs.HasKey("ObjectPath"))
        {
            ...
            editorInfomation = AssetDatabase.LoadAssetAtPath(EditorPrefs.GetString("DatabasePath") + "/Editor/EditorInformation.asset", typeof(EditorInfomation)) as EditorInfomation;
            itemViewIndex = 1;
    ...
    private void OnGUI()
    {
        ...
        if (showPosition5)
        {
            ...
            EditorGUI.BeginChangeCheck();
            editorInfomation.labelColor = EditorGUILayout.ColorField("Highlight Label Color", editorInfomation.labelColor);
            if (EditorGUI.EndChangeCheck())
            {
                tipLabelStyle.normal.textColor = editorInfomation.labelColor;
            }
    ...
    void OpenItemList()
    {
        ...
        if (absPath.StartsWith(Application.dataPath))
        {
            ...
            {
                EditorPrefs.SetString("ObjectPath", relPath);
                EditorPrefs.SetString("DatabasePath", relPath.Substring(0, relPath.Length - (inventoryItemList.name.Length + 1 + ".asset".Length)));
            }
        }
    }
    #endregion
    ...
    void DrawItemEditor()
    {
        ...
        {
            editorInfomation.AutoChangeSprite = EditorGUILayout.Toggle("Auto Change Sprite", editorInfomation.AutoChangeSprite);
            GUILayout.Space(10);
            editorInfomation.createRigidbody2D = EditorGUILayout.Toggle("Create Rigidbody 2D", editorInfomation.createRigidbody2D);
            GUILayout.Space(10);
        }
        ...
        if (inventoryItemList != null)
        {
                ...
                if (GUILayout.Button("Prev", GUILayout.ExpandWidth(false)))
                {
                    ...
                    else
                    {
                        for (int i = 0; i < inventoryItemList.itemList.Count; i++)
                        {
                            ...
                            if (inventoryItemList.itemList[itemViewIndex - 1].variety.ToString() == m_ItemShownType.ToString())
                                break;
                        }
                    }
                    InitializeReorderableList();
            ...
            if (inventoryItemList.itemList.Count > 0)
            {
                ...
                itemViewIndex = Mathf.Clamp(EditorGUILayout.IntField("Current Item", itemViewIndex, GUILayout.ExpandWidth(false)), 1, inventoryItemList.itemList.Count);
                EditorGUILayout.LabelField("of   " + inventoryItemList.itemList.Count.ToString() + "  items", "", GUILayout.ExpandWidth(false));
                GUILayout.EndHorizontal();
                GUILayout.Space(5);

                if (GUILayout.Button("Show Item In Assets", GUILayout.ExpandWidth(false)) && inventoryItemList.itemList[itemViewIndex - 1].m_object)
                {
                ...
                if (showPosition4)
                {
                    ...
                    //下面是各种物品数据的显示
                    //ID
                    GUILayout.Label("Item ID:                         " + inventoryItemList.itemList[itemViewIndex - 1].ID.ToString());

                    //Name
                    EditorGUI.BeginChangeCheck();
                    inventoryItemList.itemList[itemViewIndex - 1].itemName = EditorGUILayout.DelayedTextField("Item Name", inventoryItemList.itemList[itemViewIndex - 1].itemName as string);
                    if (EditorGUI.EndChangeCheck())
                    {
                        AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/Items/" + inventoryItemList.itemList[itemViewIndex - 1].m_object.name + ".prefab", inventoryItemList.itemList[itemViewIndex - 1].itemName);
                        AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/Items/ItemScriptObjects/" + inventoryItemList.itemList[itemViewIndex - 1].name + ".asset", inventoryItemList.itemList[itemViewIndex - 1].itemName);
                    ...
                    inventoryItemList.itemList[itemViewIndex - 1].description = EditorGUILayout.TextArea(inventoryItemList.itemList[itemViewIndex - 1].description, GUILayout.ExpandHeight(true));
                    EditorGUILayout.EndScrollView();
                    GUILayout.Space(5);

                    //icon
                    EditorGUI.BeginChangeCheck();
                    inventoryItemList.itemList[itemViewIndex - 1].icon = EditorGUILayout.ObjectField("Item Icon", inventoryItemList.itemList[itemViewIndex - 1].icon, typeof(Sprite), false) as Sprite;
                    if (EditorGUI.EndChangeCheck())
                    {
                        if (editorInfomation.AutoChangeSprite)
                            if (inventoryItemList.itemList[itemViewIndex - 1].m_object && inventoryItemList.itemList[itemViewIndex - 1].m_object.GetComponent<SpriteRenderer>())
                            {
                                DestroyImmediate(inventoryItemList.itemList[itemViewIndex - 1].m_object.GetComponent<PolygonCollider2D>(), true);
                                inventoryItemList.itemList[itemViewIndex - 1].m_object.GetComponent<SpriteRenderer>().sprite = inventoryItemList.itemList[itemViewIndex - 1].icon;
                                inventoryItemList.itemList[itemViewIndex - 1].m_object.AddComponent<PolygonCollider2D>();
                                RefreshEditorProjectWindow();
                            }
                    }

                    GUILayout.BeginHorizontal();
                    inventoryItemList.itemList[itemViewIndex - 1].isStackable = EditorGUILayout.Toggle("Is Stackable", inventoryItemList.itemList[itemViewIndex - 1].isStackable, GUILayout.ExpandWidth(false));
                    GUILayout.EndHorizontal();
                    GUILayout.Space(5);
                    if (inventoryItemList.itemList[itemViewIndex - 1].isStackable)
                    {
                        inventoryItemList.itemList[itemViewIndex - 1].maxStackNumber = Mathf.Clamp((int)EditorGUILayout.IntField("maxStackNumber", inventoryItemList.itemList[itemViewIndex - 1].maxStackNumber), 1, 999);
                        GUILayout.Space(10);
                    }

                    inventoryItemList.itemList[itemViewIndex - 1].rarity = (Rarity)EditorGUILayout.EnumPopup("Item Rarity", inventoryItemList.itemList[itemViewIndex - 1].rarity);

                    GUILayout.Space(10);

                    inventoryItemList.itemList[itemViewIndex - 1].variety = (Variety)EditorGUILayout.EnumPopup("Item Variety", inventoryItemList.itemList[itemViewIndex - 1].variety);
                }
                GUILayout.Space(5);
                if (inventoryItemList.itemList[itemViewIndex - 1].variety == Variety.Consumable)
                {
                    ...
                    if (showPosition3)
                    {
                        ...
                        GUILayout.Space(5);
                        inventoryItemList.itemList[itemViewIndex - 1].coolDown = Mathf.Clamp((float)EditorGUILayout.FloatField("Cool Down", inventoryItemList.itemList[itemViewIndex - 1].coolDown), 0, 99999999);
                        if (inventoryItemList.itemList[itemViewIndex - 1].coolDown == 0)
                        {
                            inventoryItemList.itemList[itemViewIndex - 1].useOnPickUp = EditorGUILayout.Toggle("Use Immediately When picking up", inventoryItemList.itemList[itemViewIndex - 1].useOnPickUp, GUILayout.ExpandWidth(false));
                        }

                        if (inventoryItemList.itemList[itemViewIndex - 1].coolDown > 0)
                        {
                            inventoryItemList.itemList[itemViewIndex - 1].CDAllThisWhenUse = EditorGUILayout.Toggle("CD All This When Use", inventoryItemList.itemList[itemViewIndex - 1].CDAllThisWhenUse, GUILayout.ExpandWidth(false));
                            inventoryItemList.itemList[itemViewIndex - 1].CDAllConsumbleWhenUse = EditorGUILayout.Toggle("CD All Consumble When Use", inventoryItemList.itemList[itemViewIndex - 1].CDAllConsumbleWhenUse, GUILayout.ExpandWidth(false));
                        }
                        //
                        GUILayout.Space(5);
                        ...
                        if (inventoryItemList.itemList[itemViewIndex - 1].variety == Variety.Equipment || inventoryItemList.itemList[itemViewIndex - 1].variety == Variety.Consumable)
                        {
                            inventoryItemList.itemList[itemViewIndex - 1].clipOnUse = EditorGUILayout.ObjectField("Clip", inventoryItemList.itemList[itemViewIndex - 1].clipOnUse, typeof(AudioClip), false) as AudioClip;
                            GUILayout.Space(5);
                            if (inventoryItemList.itemList[itemViewIndex - 1].clipOnUse)
                            {
                                inventoryItemList.itemList[itemViewIndex - 1].playClipTimes = Mathf.Clamp((int)EditorGUILayout.IntField("Clip Play Times", inventoryItemList.itemList[itemViewIndex - 1].playClipTimes), 1, 99);
                            }
                        }
                    }
                }
                else if (inventoryItemList.itemList[itemViewIndex - 1].variety == Variety.Equipment)
                {
                    ...
                    if (showPosition3)
                    {
                        ...
                        GUILayout.Space(5);
                        inventoryItemList.itemList[itemViewIndex - 1].equipmentPart = (EquipmentPart)EditorGUILayout.EnumPopup("Equipment Part", inventoryItemList.itemList[itemViewIndex - 1].equipmentPart);
                        GUILayout.Space(5);
                        //
                ...
                if (showPosition1)
                {
                    ...
                    GUILayout.Space(5);
                    //gameobject
                    inventoryItemList.itemList[itemViewIndex - 1].m_object = EditorGUILayout.ObjectField("Item Object", inventoryItemList.itemList[itemViewIndex - 1].m_object, typeof(GameObject), false) as GameObject;
                    GUILayout.Space(10);

                    if (inventoryItemList.itemList[itemViewIndex - 1] != null && inventoryItemList.itemList[itemViewIndex - 1].m_object)
                    {
                        inventoryItemList.itemList[itemViewIndex - 1].m_object.layer = EditorGUILayout.LayerField("GameObject Layer", inventoryItemList.itemList[itemViewIndex - 1].m_object.layer);
                        GUILayout.Space(10);
                        inventoryItemList.itemList[itemViewIndex - 1].m_object.tag = EditorGUILayout.TagField("GameObject Tag", inventoryItemList.itemList[itemViewIndex - 1].m_object.tag);
                        GUILayout.Space(10);
    ...
    void AddItem()
    {
        int num = 1;
        //创建物体
        while (System.IO.File.Exists(EditorPrefs.GetString("DatabasePath") + "/Items" + "/New Item" + "(" + num.ToString() + ")" + ".prefab"))
            num++;
        ...
        {
            PrefabUtility.CreateEmptyPrefab(EditorPrefs.GetString("DatabasePath") + "/Items" + "/New Item" + "(" + num.ToString() + ")" + ".prefab");
        ...
        else
        {
            ...
            GameObject newPrefab = PrefabUtility.CreatePrefab(EditorPrefs.GetString("DatabasePath") + "/Items" + "/New Item" + "(" + num.ToString() + ")" + ".prefab", go);
            DestroyImmediate(go);
    ...
    void DeleteItem(int index)
    {
        ...
        if (inventoryItemList.itemList.Count > 0)
        {
            ...
            System.IO.File.Delete(EditorPrefs.GetString("DatabasePath") + "/Items/ItemScriptObjects/" + assetName + ".asset");
            System.IO.File.Delete(EditorPrefs.GetString("DatabasePath") + "/Items/ItemScriptObjects/" + assetName + ".asset.meta");
    ...
    {
        GameObjectData measureData = inventoryItemList.itemList[itemViewIndex - 1].m_object.GetComponent<GameObjectData>();
        if (measureData == null)
        {
    ...
    void DrawCraftingEditor()
    {
        ...
        if (inventoryItemList.bluePrints.Count > 0)
        {
            ...
            if (inventoryItemList.bluePrints.Count > 0)
            {
                ...
                bluePrintViewIndex = Mathf.Clamp(EditorGUILayout.IntField("Current BluePrint", bluePrintViewIndex, GUILayout.ExpandWidth(false)), 1, inventoryItemList.bluePrints.Count);
                ...
                //name
                EditorGUI.BeginChangeCheck();
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].bluePrintName = EditorGUILayout.DelayedTextField("BluePrint Name", inventoryItemList.bluePrints[bluePrintViewIndex - 1].bluePrintName as string);
                if (EditorGUI.EndChangeCheck())
                {
                    AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/BluePrints/" + inventoryItemList.bluePrints[bluePrintViewIndex - 1].name + ".asset", inventoryItemList.bluePrints[bluePrintViewIndex - 1].bluePrintName);
                    AssetDatabase.SaveAssets();
                }
                GUILayout.Space(5);
                //icon
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].icon = EditorGUILayout.ObjectField("BluePrint Icon", inventoryItemList.bluePrints[bluePrintViewIndex - 1].icon, typeof(Sprite), false) as Sprite;
                GUILayout.Space(5);
                //target
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].targetItem = EditorGUILayout.ObjectField("Product", inventoryItemList.bluePrints[bluePrintViewIndex - 1].targetItem, typeof(Item), false) as Item;
                GUILayout.Space(5);
                //amount after crafting
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].craftingAmount = Mathf.Clamp(EditorGUILayout.IntField("Product Amount", inventoryItemList.bluePrints[bluePrintViewIndex - 1].craftingAmount), 0, 9999999);
                GUILayout.Space(5);
                //Success Chance
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].successChance = Mathf.Clamp(EditorGUILayout.FloatField("Success Chance", inventoryItemList.bluePrints[bluePrintViewIndex - 1].successChance), 0, 1);
                GUILayout.Space(5);
                //Crafting Time
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].craftingTime = Mathf.Clamp(EditorGUILayout.FloatField("Crafting Time", inventoryItemList.bluePrints[bluePrintViewIndex - 1].craftingTime), 0, 9999999);
                GUILayout.Space(5);
                //CDAll
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].CDAllWhenCrafting = EditorGUILayout.Toggle("CD All When Crafting", inventoryItemList.bluePrints[bluePrintViewIndex - 1].CDAllWhenCrafting, GUILayout.ExpandWidth(false));
                GUILayout.Space(5);
                //move after crafting
                inventoryItemList.bluePrints[bluePrintViewIndex - 1].moveAfterCrafting = EditorGUILayout.Toggle("Moved After Crafting", inventoryItemList.bluePrints[bluePrintViewIndex - 1].moveAfterCrafting, GUILayout.ExpandWidth(false));
                GUILayout.Space(10);
    ...
    void DrawCurrencyEditor()
    {
        ...
        if (inventoryItemList.currencies.Count > 0)
        {
            ...
            if (inventoryItemList.currencies.Count > 0)
            {
                ...
                currencyViewIndex = Mathf.Clamp(EditorGUILayout.IntField("Current Currency", currencyViewIndex, GUILayout.ExpandWidth(false)), 1, inventoryItemList.currencies.Count);
                ...
                //name
                EditorGUI.BeginChangeCheck();
                inventoryItemList.currencies[currencyViewIndex - 1].currencyName = EditorGUILayout.DelayedTextField("Currency Name", inventoryItemList.currencies[currencyViewIndex - 1].currencyName as string);
                if (EditorGUI.EndChangeCheck())
                {
                    AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/Currencies/" + inventoryItemList.currencies[currencyViewIndex - 1].name + ".asset", inventoryItemList.currencies[currencyViewIndex - 1].currencyName);
                    AssetDatabase.SaveAssets();
                }
                GUILayout.Space(5);
                //icon
                inventoryItemList.currencies[currencyViewIndex - 1].icon = EditorGUILayout.ObjectField("Currency Icon", inventoryItemList.currencies[currencyViewIndex - 1].icon, typeof(Sprite), false) as Sprite;
                GUILayout.Space(5);

                //exchangerate
                inventoryItemList.currencies[currencyViewIndex - 1].exchangeRate = Mathf.Clamp(EditorGUILayout.FloatField("Exchange Rate", inventoryItemList.currencies[currencyViewIndex - 1].exchangeRate), 0.000001f, 9999999);
                GUILayout.Space(5);
                GUILayout.Space(20);
            }
    ...
    void DrawAttributeEditor()
    {
        ...
        if (inventoryItemList.attributes.Count > 0)
        {
            ...
            if (inventoryItemList.attributes.Count > 0)
            {
                ...
                attributeViewIndex = Mathf.Clamp(EditorGUILayout.IntField("Current Attribute", attributeViewIndex, GUILayout.ExpandWidth(false)), 1, inventoryItemList.attributes.Count);
                ...
                //name
                EditorGUI.BeginChangeCheck();
                inventoryItemList.attributes[attributeViewIndex - 1].attributeName = EditorGUILayout.DelayedTextField("Attribute Name", inventoryItemList.attributes[attributeViewIndex - 1].attributeName as string);
                if (EditorGUI.EndChangeCheck())
                {
                    AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/ItemAttributes/" + inventoryItemList.attributes[attributeViewIndex - 1].name + ".asset", inventoryItemList.attributes[attributeViewIndex - 1].attributeName);
                    AssetDatabase.SaveAssets();
                }
                GUILayout.Space(5);
                //icon
                inventoryItemList.attributes[attributeViewIndex - 1].icon = EditorGUILayout.ObjectField("Attribute Icon", inventoryItemList.attributes[attributeViewIndex - 1].icon, typeof(Sprite), false) as Sprite;
