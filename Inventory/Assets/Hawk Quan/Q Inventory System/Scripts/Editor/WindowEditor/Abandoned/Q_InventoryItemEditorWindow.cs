            ...
            using (var scrollView = new EditorGUILayout.ScrollViewScope(scrollPos))
            {
                ...
                if (inventoryItemList != null)
                {
                    ...
                    if (inventoryItemList.itemList.Count > 0)
                    {
                        ...
                        //下面是各种物品数据的显示
                        //ID
                        GUILayout.Label("Item ID:                         " + inventoryItemList.itemList[viewIndex - 1].ID.ToString());

                        //Name
                        EditorGUI.BeginChangeCheck();
                        inventoryItemList.itemList[viewIndex - 1].itemName = EditorGUILayout.DelayedTextField("Item Name", inventoryItemList.itemList[viewIndex - 1].itemName as string);
                        if(EditorGUI.EndChangeCheck())
                        {
                            AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/Items/" + inventoryItemList.itemList[viewIndex - 1].m_object.name + ".prefab", inventoryItemList.itemList[viewIndex - 1].itemName);
                            AssetDatabase.RenameAsset(EditorPrefs.GetString("DatabasePath") + "/Items/ItemScriptObjects/" + inventoryItemList.itemList[viewIndex - 1].name + ".asset", inventoryItemList.itemList[viewIndex - 1].itemName);
                        ...
                        inventoryItemList.itemList[viewIndex - 1].description = EditorGUILayout.TextArea(inventoryItemList.itemList[viewIndex - 1].description, GUILayout.ExpandHeight(true));
                        EditorGUILayout.EndScrollView();
                        GUILayout.Space(5);

                        //icon
                        EditorGUI.BeginChangeCheck();
                        inventoryItemList.itemList[viewIndex - 1].icon = EditorGUILayout.ObjectField("Item Icon", inventoryItemList.itemList[viewIndex - 1].icon, typeof(Sprite), false) as Sprite;
                        if (EditorGUI.EndChangeCheck())
                        {
                            if (inventoryItemList.itemList[viewIndex - 1].m_object && inventoryItemList.itemList[viewIndex - 1].m_object.GetComponent<SpriteRenderer>())
                            {
                                inventoryItemList.itemList[viewIndex - 1].m_object.GetComponent<SpriteRenderer>().sprite = inventoryItemList.itemList[viewIndex - 1].icon;
                                RefreshEditorProjectWindow();
                            }
                        }
                        //gameobject
                        inventoryItemList.itemList[viewIndex - 1].m_object = EditorGUILayout.ObjectField("Item Object", inventoryItemList.itemList[viewIndex - 1].m_object, typeof(GameObject), false) as GameObject;

                        GUILayout.Space(10);

                        GUILayout.BeginHorizontal();
                        inventoryItemList.itemList[viewIndex - 1].isStackable = EditorGUILayout.Toggle("Is Stackable", inventoryItemList.itemList[viewIndex - 1].isStackable, GUILayout.ExpandWidth(false));
                        GUILayout.EndHorizontal();
                        GUILayout.Space(5);
                        if (inventoryItemList.itemList[viewIndex - 1].isStackable)
                        {
                            inventoryItemList.itemList[viewIndex - 1].maxStackNumber = Mathf.Clamp((int)EditorGUILayout.IntField("maxStackNumber", inventoryItemList.itemList[viewIndex - 1].maxStackNumber), 1, 999);
                            GUILayout.Space(10);
                        }

                        inventoryItemList.itemList[viewIndex - 1].rarity = (Rarity)EditorGUILayout.EnumPopup("Item Rarity", inventoryItemList.itemList[viewIndex - 1].rarity);

                        GUILayout.Space(10);

                        inventoryItemList.itemList[viewIndex - 1].variety = (Variety)EditorGUILayout.EnumPopup("Item Variety", inventoryItemList.itemList[viewIndex - 1].variety);

                        if (inventoryItemList.itemList[viewIndex - 1].variety == Variety.Consumable)
                        {
                            GUILayout.Space(10);
                            GUILayout.Label("Consumable Settings", tipLabelStyle);
                            inventoryItemList.itemList[viewIndex - 1].coolDown = Mathf.Clamp((float)EditorGUILayout.FloatField("Cool Down", inventoryItemList.itemList[viewIndex - 1].coolDown), 0, 99999999);
                            //
                        ...
                        else if (inventoryItemList.itemList[viewIndex - 1].variety == Variety.Equipment)
                        {
                            ...
                            inventoryItemList.itemList[viewIndex - 1].equipmentPart = (EquipmentPart)EditorGUILayout.EnumPopup("Equipment Part", inventoryItemList.itemList[viewIndex - 1].equipmentPart);
                        ...
                        GUILayout.Space(10);
                        if(inventoryItemList.itemList[viewIndex - 1].variety == Variety.Equipment || inventoryItemList.itemList[viewIndex - 1].variety == Variety.Consumable)
                        {
                            inventoryItemList.itemList[viewIndex - 1].clipOnUse = EditorGUILayout.ObjectField("Clip", inventoryItemList.itemList[viewIndex - 1].clipOnUse, typeof(AudioClip), false) as AudioClip;
                            if(inventoryItemList.itemList[viewIndex - 1].clipOnUse)
                            {
                                inventoryItemList.itemList[viewIndex - 1].playClipTimes = Mathf.Clamp((int)EditorGUILayout.IntField("Clip Play Times", inventoryItemList.itemList[viewIndex - 1].playClipTimes), 1, 99);
                            }
                        }
