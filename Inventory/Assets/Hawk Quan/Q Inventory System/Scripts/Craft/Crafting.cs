...
public class Crafting : Q_Inventory {
    ...
    public void AddItem(CraftingBluePrint craftingBluePrint)
    {
    ...
    bool CheckIsHaveEnoughItems(List<Ingredient> ingredients, int amountToCraft)
    {
        ...
        foreach (Ingredient m_Ingredient in ingredients)
        {
            ...
            {
                //Debug.Log(amountToCraft * m_Ingredient.amount);
                //Debug.Log("Lock Item" + m_Ingredient.ingredient.name + " " + (amountToCraft * m_Ingredient.amount - tempAmount));
                return false;
            }

            //else
            //{
            //    Debug.Log(m_Ingredient.ingredient.name + "Total Ingredients Amount " + tempAmount);
            //}
        }
    ...
    IEnumerator AddCraftingItem(ItemData itemdata)
    {
        ...
        PlayerInventoryManager.UpdataPlayerCurrency(itemdata.m_CraftingBluePrint.craftingPrices, -1 * amount);
        //Debug.Log(amount);
