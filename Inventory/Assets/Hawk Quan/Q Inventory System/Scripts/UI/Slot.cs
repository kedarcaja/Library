...
public class Slot : MonoBehaviour, IDropHandler
{
    ...
    public void OnDrop(PointerEventData eventData)
    {
        ...
        if (tag == "Storage" || tag == "Inventory" || tag == "SkillBar")
        {
            ...
            else if((droppedItem.inv.tag == "Inventory" || droppedItem.inv.tag == "SkillBar") && droppedItem.inv.ReturnCoolingState(droppedItem.slot))
            {
                return;
            }
            ...
            else
            {
                ...
                {
                    if(data.item == droppedItem.item && data.item.isStackable && inv.CheckSlot(slotID) > droppedItem.amount)
                    {
                        droppedItem.ClearItem();
