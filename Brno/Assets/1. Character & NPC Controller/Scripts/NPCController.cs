...
public class NPCController : MonoBehaviour
{
    ...
    private float Range = 10; // distance in scene units below which the NPC will increase speed and seek the player
    [SerializeField]
    private Mode mode;
    [SerializeField]
    ...
    private void Update()
    {
        ...
        //if (Vector3.Distance(NPCtarget.position, player.position)>0)
        //{

        //}
        Debug.Log(Vector3.Distance(NPCtarget.position, player.position));
    }
    void Patrol()
    {
    ...
    void Tick()
    {
        switch (mode)
        {
            ...
            case Mode.GoToWaypointWithPlayer:
                ...
                if (player != null && Vector3.Distance(transform.position, player.position) > Range && pos ==true)
                {
                    agent.speed = 0f;
                }
                if (player != null && Vector3.Distance(transform.position, player.position) > Range && pos ==false)
                {
                    agent.speed = agentSpeed;
                }
                break;
