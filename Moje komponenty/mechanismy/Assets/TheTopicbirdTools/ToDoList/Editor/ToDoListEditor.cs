...
public class ToDoListEditor : Editor {
	...
	public override void OnInspectorGUI() {
		...
		for (int i = 0; i < script.textList.Count; i++) {
			...
			if (!EditorApplication.isPlaying) {
				...
				if (t.deleteQuestion){
					GUI.color = Color.red;
					if (GUILayout.Button ("ANO?", new GUIStyle(EditorStyles.miniButtonRight), GUILayout.MaxWidth(45), GUILayout.MaxHeight(14))) {
						refreshRequests();
						script.DeleteText (i);
					}
				} else {
		...
		if (!EditorApplication.isPlaying) {
			...
			if (GUILayout.Button ("Přidat item", GUILayout.MaxWidth (100), GUILayout.MinWidth (100), GUILayout.MaxHeight (25), GUILayout.MinHeight (25))) {
				refreshRequests();
			...
			// the SORT button
			GUI.color = Color.white;
			if (GUILayout.Button ("Seřadit", GUILayout.MaxWidth (60), GUILayout.MinWidth (45), GUILayout.MaxHeight (25), GUILayout.MinHeight (25))) {
				refreshRequests();
				script.SortText ();
			}
			EditorGUILayout.EndHorizontal ();
	...
	Color getColor(ToDoList.TextObject.State state){
		Color c = Color.white;
		if (state == ToDoList.TextObject.State.Vprocesu){
			c = new Color(1f, 0.85f, 0.4f);
		} else if (state == ToDoList.TextObject.State.Dokončený){
			c = new Color(0.70f, 1f, 0.50f);
		} else if (state == ToDoList.TextObject.State.Bug){
			c = new Color(1f, 0f,0f, 1f);
		} else if (state == ToDoList.TextObject.State.Odložený){
